...
class DualSpaceKDWithCMA_OT_2(VariousDivergence):
    def __init__(self, args, padding_id=-100):
        super().__init__(args, padding_id=padding_id)
        print("--------------------Using KB Su dung Multi-OT-New-Ablation-2-------------------")
        self.args = args
    ...
    def forward(
        ...
        total_loss = self.ce_ * loss_ce + self.kd_rate * kd_loss + self.ot_weight_logits * ot_loss_logits
        log["loss"] = total_loss
...
#     def __init__(self, args, padding_id=-100):
#         super().__init__(args, padding_id=padding_id)
#         print("--------------------Using KB Su dung Multi-OT-KB2-------------------")
#         self.args = args
#         self.etp = ETP_1()
...
#         self.tau_seq = 2.0
#         self.lambda_sinkhorn = 0.2
#         self.k_truncation = 50
#         self.ot_weight = 150.0
#         self.sinkhorn_iterations = 10

#         self.cost_weights_last = nn.Parameter(torch.tensor([0.15, 0.3, 0.2, 0.2, 0.15], dtype=self.dtype, device=self.device))

#         self._ngram_cache = {}
#         self.top_k_vocab = 500  # Giảm vocab_size
...
#             distiller, outputs, teacher_outputs, log, pad_mask, teacher_pad_mask, is_last_layer=True
#         )

#         # ot_loss_first, log = self.compute_etp_loss(
#         #     distiller, hidden_state_student_first, teacher_outputs, 
#         #     pad_mask, teacher_pad_mask, log, is_last_layer=False
#         # )

#         ot_loss = ot_loss_last
...
#         self, distiller, student_outputs, teacher_outputs, log, 
#         attention_mask_student, attention_mask_teacher,
#         logits=False, is_last_layer=True
#     ):
#         """
...
#         """
#         # batch_size = teacher_outputs.size(0)
#         batch_size = student_outputs.logits.size(0) if is_last_layer else student_outputs.size(0)
#         total_loss = 0
#         eps = 1e-7
#         if is_last_layer:
#             # Logits
...
#                 teacher_probs = teacher_probs.index_select(-1, distiller.tea2stu_id_mapping)
#             else:
#                 min_vocab = min(student_probs.size(-1), teacher_probs.size(-1))
#                 student_probs = student_probs[:, :, :min_vocab]
#                 teacher_probs = teacher_probs[:, :, :min_vocab]

#             # Top-k probabilities
#             if self.top_k_vocab < student_probs.size(-1):
#                 _, topk_indices = torch.topk(student_probs, k=self.top_k_vocab, dim=-1)
#                 student_probs = torch.gather(student_probs, dim=-1, index=topk_indices)
#                 teacher_probs = torch.gather(teacher_probs, dim=-1, index=topk_indices)

#             # Handle variable sequence lengths
#             max_seq_len_s = student_probs.size(1)
#             max_seq_len_t = teacher_probs.size(1)
#             student_probs_list = []
#             teacher_probs_list = []
#             for b in range(batch_size):
#                 mask_s = attention_mask_student[b].bool()
#                 mask_t = attention_mask_teacher[b].bool()
#                 student_probs_b = student_probs[b][mask_s]
#                 teacher_probs_b = teacher_probs[b][mask_t]
#                 student_probs_list.append(student_probs_b)
#                 teacher_probs_list.append(teacher_probs_b)
            
#             # Pad to max length
#             max_M = max(p.size(0) for p in teacher_probs_list)
#             max_N = max(p.size(0) for p in student_probs_list)
#             student_probs_padded = torch.zeros(batch_size, max_N, student_probs.size(-1), device=self.device, dtype=self.dtype)
#             teacher_probs_padded = torch.zeros(batch_size, max_M, student_probs.size(-1), device=self.device, dtype=self.dtype)
#             for b in range(batch_size):
#                 student_probs_padded[b, :student_probs_list[b].size(0)] = student_probs_list[b]
#                 teacher_probs_padded[b, :teacher_probs_list[b].size(0)] = teacher_probs_list[b]

#             # Debug shapes
#             # print(f"max_M: {max_M}, max_N: {max_N}, vocab_size: {student_probs.size(-1)}")
#             # print(f"teacher_probs_padded shape: {teacher_probs_padded.shape}")
#             # print(f"student_probs_padded shape: {student_probs_padded.shape}")

#             # C_had
#             C_had = torch.zeros(batch_size, max_M, max_N, device=self.device, dtype=self.dtype)
#             for b in range(batch_size):
#                 teacher_b = teacher_probs_padded[b]  # (max_M, vocab_size)
#                 student_b = student_probs_padded[b]  # (max_N, vocab_size)
#                 # print(f"Batch {b}: teacher_b shape: {teacher_b.shape}, student_b shape: {student_b.shape}")
#                 diff = teacher_b.unsqueeze(1) - student_b.unsqueeze(0)  # (max_M, max_N, vocab_size)
#                 # print(f"Batch {b}: diff shape: {diff.shape}")
#                 C_had[b] = torch.abs(diff).mean(dim=-1)  # (max_M, max_N)
#             log["avg_c_had_last"] = C_had.mean().item()

#             # C_sl
#             # C_sl = torch.zeros(batch_size, max_M, max_N, device=self.device, dtype=self.dtype)
#             # for b in range(batch_size):
#             #     teacher_b = teacher_probs_padded[b]
#             #     student_b = student_probs_padded[b]
#             #     print(f"Batch {b}: teacher_b shape: {teacher_b.shape}, student_b shape: {student_b.shape}")
#             #     C_sl[b] = -teacher_b.unsqueeze(1) * torch.log(student_b.unsqueeze(0) + eps)
#             #     print(f"Batch {b}: C_sl[b] shape before mean: {C_sl[b].shape}")
#             #     C_sl[b] = C_sl[b].mean(dim=-1)
#             #     print(f"Batch {b}: C_sl[b] shape after mean: {C_sl[b].shape}")
#             # log["avg_c_sl_last"] = C_sl.mean().item()

#             # C_seq
#             teacher_probs_fp32 = teacher_probs_padded.to(torch.float32)
#             student_probs_fp32 = student_probs_padded.to(torch.float32)
#             C_seq = torch.cdist(teacher_probs_fp32, student_probs_fp32, p=1).to(self.dtype)
#             # K = torch.exp(-C_seq / self.lambda_sinkhorn)
#             # for _ in range(self.sinkhorn_iterations):
#             #     K = K / (K.sum(dim=2, keepdim=True) + eps)
#             #     K = K / (K.sum(dim=1, keepdim=True) + eps)
#             # P_lambda = K
#             log["avg_c_seq_last"] = C_seq.mean().item()
#             # log["loss_sd_last"] = (P_lambda * C_seq).sum().item()

#             # C2: Simplified edit distance
#             C2 = torch.zeros(batch_size, max_M, max_N, device=self.device, dtype=self.dtype)
#             for b in range(batch_size):
#                 stu_ids = torch.argmax(student_probs_list[b], dim=-1).tolist()
#                 tea_ids = torch.argmax(teacher_probs_list[b], dim=-1).tolist()
#                 stu_tok = distiller.student_tokenizer.convert_ids_to_tokens(stu_ids, skip_special_tokens=True)
#                 tea_tok = distiller.student_tokenizer.convert_ids_to_tokens(tea_ids, skip_special_tokens=True)
#                 min_len = min(len(stu_tok), len(tea_tok))
#                 for i in range(min_len):
#                     if i < max_N and i < max_M:
#                         C2[b, i, i] = editdistance.eval(stu_tok[i], tea_tok[i])
#             log["avg_c2_last"] = C2.mean().item()

#             # C4: Contextual KL-divergence (using einsum to avoid broadcasting)
#             def compute_context_probs(probs, window=2):
#                 ctx = torch.zeros_like(probs)
#                 for i in range(probs.size(1)):
#                     start = max(i - window, 0)
#                     end = min(i + window + 1, probs.size(1))
#                     ctx[:, i] = probs[:, start:end].mean(dim=1)
#                 return ctx
#             ctx_s = compute_context_probs(student_probs_padded)
#             ctx_t = compute_context_probs(teacher_probs_padded)
#             C4 = torch.zeros(batch_size, max_M, max_N, device=self.device, dtype=self.dtype)
#             for b in range(batch_size):
#                 teacher_b = ctx_t[b]  # (max_M, vocab_size)
#                 student_b = ctx_s[b]  # (max_N, vocab_size)
#                 # print(f"Batch {b}: ctx_t[b] shape: {teacher_b.shape}, ctx_s[b] shape: {student_b.shape}")
#                 C4[b] = torch.einsum('mv,nv->mn', -teacher_b, torch.log(student_b + eps))  # (max_M, max_N)
#                 # print(f"Batch {b}: C4[b] shape: {C4[b].shape}")
#             log["avg_c4_last"] = C4.mean().item()

#             # C5: Contextual Euclidean distance
#             ctx_s_fp32 = ctx_s.to(torch.float32)
#             ctx_t_fp32 = ctx_t.to(torch.float32)
#             C5 = torch.cdist(ctx_t_fp32, ctx_s_fp32, p=2).to(self.dtype) / 40.0
#             log["avg_c5_last"] = C5.mean().item()

#             # Combine
#             cost_matrices = [C_had, C_seq, C2, C4, C5]
...
#     def update_cost_weights(self, cost_values_last):
