...
def find_lcs_word_indices(words1, words2):
    """Tìm LCS và trả về set các cặp chỉ số từ (idx1, idx2)."""
...
def map_tokens_to_words_improved_heuristic(tokens, words, text, tokenizer):
    """
    Ánh xạ token sang từ sử dụng heuristic cải tiến hơn so với gốc.
    Cố gắng xử lý subword và khớp vị trí tốt hơn.
    Input:
        tokens: list of token strings (subwords)
        words: list of word strings
        text: the decoded text string (lowercase)
        tokenizer: the tokenizer used
    Output:
        token_to_word_map: dict mapping token_index -> word_index
    """
    token_to_word_map = {}
    current_char_idx = 0
    tok_idx = 0
    ...
    # Thử lấy offset mapping từ tokenizer nếu có (cách tốt nhất)
    # Ví dụ: tokenizer_encode_plus có thể trả về offset_mapping
    # offsets = tokenizer(text, return_offsets_mapping=True).get("offset_mapping")
    # Nếu có offsets, việc map sẽ chính xác hơn nhiều. Giả sử không có ở đây.

    word_boundaries = []
    temp_char_idx = 0
    for word in words:
        try:
            # Tìm vị trí bắt đầu và kết thúc của từ trong text gốc
            # Sử dụng regex để tìm từ hoàn chỉnh, tránh khớp một phần
            # \b là word boundary
            match = re.search(r'\b' + re.escape(word) + r'\b', text[temp_char_idx:])
            if match:
                start = temp_char_idx + match.start()
                end = temp_char_idx + match.end()
                word_boundaries.append((start, end, word_idx))
                temp_char_idx = end # Cập nhật vị trí tìm kiếm tiếp theo
            else:
                # Nếu regex không tìm thấy, thử tìm chuỗi thường (ít chính xác hơn)
                start = text.find(word, temp_char_idx)
                if start != -1:
                    end = start + len(word)
                    word_boundaries.append((start, end, word_idx))
                    temp_char_idx = end
                # else: # Bỏ qua từ nếu không tìm thấy
                    # print(f"Warning: Word '{word}' not found precisely.")
            word_idx += 1
        except Exception:
            # print(f"Error finding word '{word}'")
            word_idx += 1 # Vẫn tăng word_idx

    # Duyệt qua các token và xác định xem nó thuộc khoảng ký tự của từ nào
    current_char_idx = 0
    for i, token in enumerate(tokens):
        # Cố gắng decode token về dạng gần với text nhất có thể
        # Đây vẫn là điểm yếu nếu không có offset mapping
        try:
             # Thử decode, loại bỏ các token đặc biệt (như BOS/EOS nếu skip_special=True khi tạo text)
             # Cần đảm bảo text không chứa token đặc biệt này
            token_text = tokenizer.decode(tokenizer.convert_tokens_to_ids(token)).strip()
             # Xử lý các trường hợp đặc biệt của tokenizer nếu biết (vd: 'Ġ')
             # if token_text.startswith('Ġ'): token_text = token_text[1:]
        except:
             # Nếu không decode được (vd: token lạ), dùng string gốc
            token_text = token # Có thể cần loại bỏ '##' nếu có
             # if token_text.startswith('##'): token_text = token_text[2:]

        token_len = len(token_text)
        token_start = -1

        # Tìm vị trí của token_text trong text gốc, bắt đầu từ current_char_idx
        # Việc này vẫn có thể sai nếu token_text xuất hiện nhiều lần
        try:
            # Tìm lần xuất hiện đầu tiên của token_text từ vị trí hiện tại
            # hoặc vị trí rất gần (để xử lý lỗi nhỏ trong decode/strip)
            search_window = 10 # Tìm trong khoảng nhỏ quanh vị trí dự kiến
            start_search = max(0, current_char_idx - search_window//2)
            found_pos = text.find(token_text, start_search)

            # Heuristic: Chọn vị trí gần nhất với current_char_idx
            if found_pos != -1:
                 # Kiểm tra các lần xuất hiện khác gần đó? Không cần thiết nếu logic đúng
                token_start = found_pos
            # else: # Token không tìm thấy gần vị trí hiện tại
                 # print(f"Warning: Token '{token}' ('{token_text}') not found near char {current_char_idx}")


        except ValueError:
            # print(f"Warning: Token text '{token_text}' not found.")
            pass # Bỏ qua token này nếu không tìm được

        if token_start != -1:
            token_end = token_start + token_len
            # Tìm xem token này nằm trong word boundary nào
            assigned = False
            for w_start, w_end, w_idx in word_boundaries:
                # Token được coi là thuộc về từ nếu nó giao hoặc nằm gọn trong khoảng của từ
                # Tiêu chí có thể thay đổi: cần giao ít nhất ? ký tự, hay tâm token nằm trong?
                # Lấy tiêu chí đơn giản: có giao nhau (overlap)
                if max(token_start, w_start) < min(token_end, w_end):
                    token_to_word_map[i] = w_idx
                    assigned = True
                    break # Chỉ gán cho từ đầu tiên tìm thấy

            # Cập nhật vị trí ký tự hiện tại cho token tiếp theo
            # Nên là vị trí kết thúc của token này
            current_char_idx = token_end # Cập nhật vị trí để tìm token sau
        # else: # Nếu token không tìm thấy, không cập nhật current_char_idx?
             # Giữ nguyên current_char_idx hoặc tăng nhẹ? Giữ nguyên cho an toàn.
             # current_char_idx += 1 # Thử tăng nhẹ để tránh kẹt
...
class DualSpaceKDWithCMA_OT_3(VariousDivergence):
    def __init__(self, args, padding_id=-100):
        # Giữ nguyên __init__ như gốc, chỉ bỏ _id_mapping_cache nếu muốn
        super().__init__(args, padding_id=padding_id)
        print("--------------------Using KB Su dung Multi-OT-New (Improved LCS Heuristic)-------------------")
        self.args = args
        ...
        self.ce_ = 10.0
        self.kd_rate = 2.5  # Thu 2.0 thi duoc 18.7; thu 0.9 thi te hon, 3.0 va 15.0 ma ko co CE_KD thi rat te (14.0); 3.0 va 15 co CE te 17. 2.5 voi 5.0 300 thi ko tot bang 10.0 300
        self.tau_seq = 1.7
    ...
    def compute_ot_hidden(self, distiller, student_outputs, teacher_outputs, attention_mask_student, attention_mask_teacher, log):
        ...
        for b in range(batch_size):
            ...
            C7 = torch.ones((N, M), device=student_seq.device, dtype=self.dtype)
            ...
            except Exception: word_tokens_stu, word_tokens_tea = [], []

            if word_tokens_stu and word_tokens_tea:
                lcs_word_indices = find_lcs_word_indices(word_tokens_stu, word_tokens_tea)
                stu_word_map = map_tokens_to_words_improved_heuristic(
                    stu_tok, word_tokens_stu, stu_text, distiller.student_tokenizer
                )
                tea_word_map = map_tokens_to_words_improved_heuristic(
                    tea_tok, word_tokens_tea, tea_text, distiller.teacher_tokenizers[distiller.teacher_model_type]
                )

                for i in range(N):
                    for j in range(M):
                        k_s = stu_word_map.get(i)
                        k_t = tea_word_map.get(j)
                        if k_s is not None and k_t is not None and (k_s, k_t) in lcs_word_indices:
                            C7[i, j] = 0.0
