...
def finetune(
    ...
    cost_values_last_buffer = []
    cost_values_first_buffer = []
    ...
    for epoch in range(args.num_epochs):
        ...
        while True:
            ...
            for batch in global_batch:
                ...
                # Add
                required_last_keys = ["avg_c_hybrid_last", "avg_c3_last", "avg_c4_last", "avg_c5_last", "avg_c6_last"]
                required_first_keys = ["avg_c2_first", "avg_c5_first"]

                if all(k in batch_logging_output for k in required_last_keys + required_first_keys):
                    def to_scalar(value):
                    ...
                    try:
                        ...
                        cost_values_last = [
                            ...
                            to_scalar(batch_logging_output["avg_c_hybrid_last"]),
                            to_scalar(batch_logging_output["avg_c3_last"]),
                            to_scalar(batch_logging_output["avg_c4_last"]),
                            to_scalar(batch_logging_output["avg_c5_last"]),
                            to_scalar(batch_logging_output["avg_c6_last"])
                        ]
                        cost_values_first = [
                            to_scalar(batch_logging_output["avg_c2_first"]),
                            to_scalar(batch_logging_output["avg_c5_first"])
                        ]
                    except ValueError as e:
                        logger.error(f"Failed to convert logging_output to scalar: {e}, values: {batch_logging_output}")
                        continue
                    cost_values_last_buffer.append(cost_values_last)
                    cost_values_first_buffer.append(cost_values_first)
                else:
            ...
            if hasattr(criterion, "update_cost_weights") and step_since_last_update >= update_interval:
                ...
                if cost_values_last_buffer and cost_values_first_buffer:
                    expected_buffer_size = update_interval * args.gradient_accumulation_steps
                    if len(cost_values_last_buffer) != expected_buffer_size:
                        print(f"Unexpected cost_values_last_buffer size: {len(cost_values_last_buffer)}, expected {expected_buffer_size}")
                        cost_values_last_buffer = []
                        cost_values_first_buffer = []
                        continue

                    cost_values_last = torch.tensor(cost_values_last_buffer).mean(dim=0)
                    cost_values_first = torch.tensor(cost_values_first_buffer).mean(dim=0)

                    criterion.update_cost_weights(
                            cost_values_last=cost_values_last.tolist(),
                            cost_values_first=cost_values_first.tolist()
                        )
                    cost_values_last_buffer = []
                    cost_values_first_buffer = []
